#!/usr/bin/env python3
"""Open filtered Mitchell Martin job pages in browser tabs."""
...
def latest_jobs_file(output_dir: Path) -> Path:
    files = sorted(output_dir.glob("mitchellmartin_jobs_*.json"), key=lambda path: path.stat().st_mtime, reverse=True)
    if not files:
        raise FileNotFoundError(f"No mitchellmartin_jobs_*.json files found in {output_dir}")
    return files[0]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Open Mitchell Martin job pages.")
    ...
    print(f"Opening {len(selected)} Mitchell Martin tabs...")
