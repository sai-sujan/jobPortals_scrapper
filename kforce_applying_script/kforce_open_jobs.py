#!/usr/bin/env python3
"""Open filtered Kforce job pages in the existing Chrome window."""
...
def latest_jobs_file(output_dir: Path) -> Path:
    files = sorted(output_dir.glob("kforce_jobs_*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not files:
        raise FileNotFoundError(f"No kforce_jobs_*.json files found in {output_dir}")
    return files[0]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Open Kforce job pages in existing Chrome.")
    parser.add_argument("--jobs-file", type=Path, help="Filtered kforce_jobs_*.json. Defaults to latest.")
...
def main() -> int:
    ...
    print(f"Opening {len(selected)} Kforce job tabs in Chrome...")
