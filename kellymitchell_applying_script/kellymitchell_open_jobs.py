#!/usr/bin/env python3
"""Open filtered KellyMitchell job pages in browser tabs."""
...
def latest_jobs_file(output_dir: Path) -> Path:
    files = sorted(output_dir.glob("kellymitchell_jobs_*.json"), key=lambda path: path.stat().st_mtime, reverse=True)
    if not files:
        raise FileNotFoundError(f"No kellymitchell_jobs_*.json files found in {output_dir}")
    return files[0]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Open KellyMitchell job pages.")
    ...
    print(f"Opening {len(selected)} KellyMitchell tabs...")
