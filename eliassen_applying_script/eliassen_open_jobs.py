#!/usr/bin/env python3
"""Open filtered Eliassen job pages in the existing browser."""
...
def latest_jobs_file(output_dir: Path) -> Path:
    files = sorted(output_dir.glob("eliassen_jobs_*.json"), key=lambda path: path.stat().st_mtime, reverse=True)
    if not files:
        raise FileNotFoundError(f"No eliassen_jobs_*.json files found in {output_dir}")
    return files[0]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Open Eliassen job pages in browser tabs.")
    parser.add_argument("--jobs-file", type=Path, help="Filtered eliassen_jobs_*.json. Defaults to latest output.")
...
def main() -> int:
    ...
    print(f"Opening {len(selected)} Eliassen tabs...")
